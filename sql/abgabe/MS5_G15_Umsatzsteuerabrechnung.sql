...
--                                                Skonto, überschreibt, STUB
--                                                [Flag SKONTO_BERECHTIGT_YN da,
--                                                 finaler Steuerbetrag fehlt])
--     - list_views.V_LIST_SUPPLIER_INVOICE     (Eingangsrechnungen aus G4, AKTIV
--                                                seit 23.06.; aktuell 0 Datenzeilen)
--
-- IDEMPOTENZ:
...
-- ----------------------------------------------------------------------------
-- list_views.V_LIST_G15_OUTPUT_VAT — Umsatzsteuer aus allen Ausgangsrechnungen.
-- (Interne Doku-Quelle: sql/02_views/002_v_list_g15_output_vat.sql)
-- ----------------------------------------------------------------------------
...
-- sp_G15_create_vat_statement (Interne Doku-Quelle: ADR-010, Quelle list_views.V_LIST_G15_VAT_SKONTO).
-- 
...
-- (Interne Doku-Quelle: sql/02_views/007_v_list_g15_vat_skonto.sql)
-- ----------------------------------------------------------------------------
-- G8 liefert je Zahlungseingang den FINALEN Steuerbetrag der Rechnung nach
-- Skonto. sp_G15_create_vat_statement überschreibt damit den ursprünglichen
-- TAX_AMOUNT (Match über INVOICE_ID), wenn IS_SKONTO = 'Y'.
-- STUB (WHERE 1 = 0): G8 liefert nur das Skonto-Flag SKONTO_BERECHTIGT_YN
-- (char(1), Stand 23.06.), aber noch KEINEN finalen Steuerbetrag nach Skonto.
-- Ohne den koennen wir TAX_AMOUNT nicht ueberschreiben (Bring-Schuld G8, Issue #26).
-- Aktivieren, sobald G8 den finalen Steuerbetrag liefert:
--   SELECT g8.INVOICE_ID, g8.<FINALER_STEUERBETRAG>, g8.SKONTO_BERECHTIGT_YN
...
-- ----------------------------------------------------------------------------
-- list_views.V_LIST_G15_INPUT_VAT — Vorsteuer aus G4 (AKTIV seit 23.06.)
-- (Interne Doku-Quelle: sql/02_views/003_v_list_g15_input_vat.sql)
-- ----------------------------------------------------------------------------
-- G4 liefert list_views.V_LIST_SUPPLIER_INVOICE (Lieferantenrechnungen) mit
-- INVOICE_ID, INVOICE_DATE, TOTAL_VAT_AMOUNT. Wir lesen TOTAL_VAT_AMOUNT direkt
-- als TAX_AMOUNT (keine Eigenberechnung, ADR-008). Achtung: View heisst
-- V_LIST_SUPPLIER_INVOICE (ohne G04-Praefix), Spalte TOTAL_VAT_AMOUNT.
-- Stand 23.06.: View vorhanden, aber 0 Datenzeilen -> liefert automatisch,
-- sobald G4 Rechnungsdaten befuellt.
...
-- ----------------------------------------------------------------------------
-- list_views.V_LIST_G15_VAT_STATEMENT — Anzeige-View Abrechnungskoepfe.
-- (Interne Doku-Quelle: sql/02_views/004_v_list_g15_vat_statement.sql)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- list_views.V_LIST_G15_VAT_STATEMENT_ITEM — Anzeige-View Beleg-Items.
-- (Interne Doku-Quelle: sql/02_views/005_v_list_g15_vat_statement_item.sql)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- list_views.V_LIST_G15_VAT_USER — Login + Rollen-Demo (keine Passwörter).
-- (Interne Doku-Quelle: sql/02_views/006_v_list_g15_vat_user.sql)
-- ----------------------------------------------------------------------------
...
-- (Interne Doku-Quelle: sql/03_stored_func/001_fn_g15_check_vat_period.sql)
-- ----------------------------------------------------------------------------
...
-- (Interne Doku-Quelle: sql/03_stored_func/002_fn_g15_calculate_vat_balance.sql)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Security-Level-Pruefung: zentrale Architektur-Funktion
-- dbo.fn_get_user_securitylevel
-- ----------------------------------------------------------------------------
-- Gruppe 15 legt KEINE eigene Security-Level-Funktion mehr an. Die zentrale
-- Funktion dbo.fn_get_user_securitylevel (Architekt, liest
-- dbo.T_USER.SECURITYLEVEL) existiert bereits in ERPDEV26S und wird von den
-- Status-Procedures direkt aufgerufen. (Coaching-Feedback Prof. Lehmann:
-- vorhandene Funktion einbinden, eigene entfernen.)
...
-- ----------------------------------------------------------------------------
-- stored_proc.sp_G15_create_vat_statement — Anlegen / Neuberechnen einer Periode.
-- (Interne Doku-Quelle: sql/04_stored_proc/001_sp_g15_create_vat_statement.sql)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
CREATE OR ALTER PROCEDURE stored_proc.sp_G15_reject_vat_statement
    @statement_id INT,
...
BEGIN
    ...
    DECLARE @actual_security_level INT = dbo.fn_get_user_securitylevel(@rejected_by);
    IF @actual_security_level IS NULL
    BEGIN
        THROW 50020, 'Unbekannter Benutzer fuer Umsatzsteuerabrechnung.', 1;
        RETURN;
...
--       -> V_LIST_G15_OUTPUT_VAT bleibt unveraendert, liefert dann automatisch alle.
--   - G4: V_LIST_SUPPLIER_INVOICE ist seit 23.06. angebunden (V_LIST_G15_INPUT_VAT
--       aktiv). Offen nur noch: G4 muss Rechnungsdaten einspielen (aktuell 0 Zeilen).
--   - G8 ergaenzt finalen Steuerbetrag nach Skonto in V_LIST_G08_PAYMENT_RECEIPT
