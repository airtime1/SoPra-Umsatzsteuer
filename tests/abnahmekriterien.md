...
| **SPC-5** | Abgeschlossene Abrechnung (`PAID`) ist unveränderlich. | Versuch, eine `PAID`-Abrechnung mit `stored_proc.sp_create_vat_statement` für dieselbe Periode zu überschreiben, liefert Fehler 50002. Direkte Tabellen-Updates sind nicht Teil des Streamlit-Flows und müssen über DB-Rechte/Architekt geregelt werden. | `tests/sql/sp_status_workflow_roles_demo.sql` |
...
| Bereich | Stand |
|---|---|
| Saldo-/Typberechnung | BESTANDEN in Sandbox mit `fn_calculate_vat_balance_basic.sql` |
| Demo-Anlage 2026-04 | BESTANDEN in Sandbox mit `sp_create_vat_statement_demo.sql` |
...
| Noch offen | Explizite SQL-Tests für `fn_check_vat_period`, Rückgabe `APPROVED -> DRAFT`, echte Partnerdaten-Szenarien für Storno/Teilstorno |
