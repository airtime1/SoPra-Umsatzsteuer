...
- MS4 erwähnt nicht explizit `ins_views` / `upd_views` für Frontend-Schreibzugriff. Da `ERP_REMOTE_USER` nur read/write/execute hat, brauchen wir vermutlich Wrapper-Views. → Stand jetzt über Stored Procs gelöst (`sp_G15_create_vat_statement`, `sp_approve_*`, `sp_pay_*`, `sp_reject_*`), Insert/Update-Views aktuell nicht nötig.
