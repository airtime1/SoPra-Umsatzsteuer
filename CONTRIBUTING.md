...
Branch von `main` abzweigen, klein halten, oft mergen.
...
1. **`CLAUDE.md` ist der zentrale Briefingpunkt.** Agent liest sie automatisch. Halte sie aktuell.
2. **Sag dem Agent explizit, dass es ein Team-Repo ist** — er soll keine direkten Commits auf `main`, keine Force-Pushes, keine `.env`-Aktionen.
3. **Lasse Agents PRs erstellen, nicht direkt mergen.** Reviews schreibt der Mensch.
4. **ADRs vor jeder größeren Architektur-Änderung** durch den Agent — bitte ein ADR in `docs/entscheidungen/` aufmachen, bevor Code geschrieben wird.
5. **Agent-Sessions sind nicht selbstdokumentierend.** Wichtige Erkenntnisse aus einer Session immer in ADR oder `docs/offene_fragen.md` festhalten.
...
- Architektur-Hintergrund: `CLAUDE.md` und `docs/entscheidungen/`
